...
class EndoObsEval(BaseEval):
    def __init__(self, cfg, **kwargs):
        ...
        self.endo_obs_col_names = self.env.endo_obs_col_names
        self.endo_inds = self.env.endo_inds
        self.raw_endo_obs = {}
        for col_name in self.endo_obs_col_names:
    ...
    def do_eval(self, **kwargs) -> None:
        ...
        for transition in transitions:
            ...
            for i in range(len(self.endo_obs_col_names)):
                col_name = self.endo_obs_col_names[i]
                ind = self.endo_inds[i]
                self.raw_endo_obs[col_name].append(curr_obs[ind])
