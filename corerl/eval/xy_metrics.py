...
class XYTableProtocol(Protocol):
    def write(
        self,
        agent_step: int,
        metric: str,
...
@config()
class XYDBConfig(BufferedWriterConfig):
    name : Literal['db'] = 'db'
...
class XYTable(BufferedWriter[_XYPoint]):
    """
    Like MetricsTable, but for two dimensional data.
    """
    def __init__(
        self,
        cfg: XYDBConfig,
    ...
    def _create_table_sql(self):
        return create_tsdb_table_query(
            ...
            columns=[
                ...
                SQLColumn(name='x', type='FLOAT', nullable=False),
                SQLColumn(name='y', type='FLOAT', nullable=False),
            ],
    ...
    def _insert_sql(self):
        return text(f"""
            ...
            (time, agent_step, metric, x, y)
            VALUES (TIMESTAMP WITH TIME ZONE :timestamp, :agent_step, :metric, :x, :y)
        """)


    def write(self, agent_step: int, metric: str, x: SupportsFloat, y: SupportsFloat,  timestamp: str | None = None):
        ...
        try:
            ...
            log.exception(f'Failed to write metric: {metric} {x}')
