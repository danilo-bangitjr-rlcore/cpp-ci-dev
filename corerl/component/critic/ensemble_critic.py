...
class EnsembleQCritic(BaseQ):
    def __init__(self, cfg: DictConfig, state_dim: int, action_dim: int):
        ...
        self.target = init_critic_target(cfg.critic_network, input_dim=state_action_dim, output_dim=1,
                                         critic=self.model)
...
class EnsembleVCritic(BaseV):
    def __init__(self, cfg: DictConfig, state_dim: int):
        ...
        self.target = init_critic_target(cfg.critic_network, input_dim=state_dim, output_dim=1, critic=self.model)
        self.optimizer = init_optimizer(cfg.critic_optimizer, list(self.model.parameters(independent=True)),
                                        ensemble=True)
        self.polyak = cfg.polyak
        self.target_sync_freq = cfg.target_sync_freq
