...
def pf_from_actions(actions: np.ndarray, ts: dict | None = None) -> PipelineFrame:
    ...
    pf.action_lo = pd.DataFrame({ "action_lo": np.zeros_like(actions) }, index=datetime_index)
    pf.action_hi = pd.DataFrame({ "action_hi": np.ones_like(actions) }, index=datetime_index)
