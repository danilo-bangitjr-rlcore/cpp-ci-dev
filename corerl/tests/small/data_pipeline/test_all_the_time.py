...
from tests.small.data_pipeline.test_trajectory_filter import make_test_step
...
def test_all_the_time_basic():
    """
    Tests to see if all the time tc will return the correct trajectories.
    """
    tc = make_tc(2)
    pf = make_pf(0, 3)
    pf = tc(pf)
    ...
    assert len(trajectories) == comb(3, 2)
...
def test_all_the_time_max_n_1():
    """
    Tests to see if all the time tc will return the correct trajectories.
    However, max_n_step is set to 1, so there should only be two trajectories
    """
    tc = make_tc(1)
    pf = make_pf(0, 3)
    pf = tc(pf)
...
def test_all_the_time_ts_1():
    ...
    pf_1 = make_pf(0, 2)
    pf_1 = tc(pf_1)
    trajectories = pf_1.trajectories
    assert isinstance(trajectories, list)
    ...
    pf_2 = tc(pf_2)
    trajectories = pf_2.trajectories
    assert isinstance(trajectories, list)
...
def test_all_the_time_online():
    """
    Tests online creation of trajectories
    """
    tc = make_tc(2, 2)
    ...
    for i in range(5):
        ...
        pf = tc(pf)
        new_trajectories = pf.trajectories
    ...
    assert trajectories[0] == expected_0
    ...
    assert trajectories[1] == expected_1
    ...
    assert trajectories[2] == expected_2
...
def test_all_the_time_data_modes():
    ...
    pf_3 = tc(pf_3)
    trajectories = pf_3.trajectories
    assert isinstance(trajectories, list)
    assert len(trajectories) == 1
