...
class SeasonalTagFeatures:
    ...
    def __call__(self, pf: PipelineFrame) -> PipelineFrame:
        ...
        if self.has_time_of_day:
            pf.data["time_of_day_sin"] = np.sin(2.0 * np.pi * pf.data[SeasonalTags.time_of_day] / 86400.0)
            pf.data["time_of_day_cos"] = np.cos(2.0 * np.pi * pf.data[SeasonalTags.time_of_day] / 86400.0)
        ...
        if self.has_day_of_year:
            ...
            pf.data["time_of_year_sin"] = np.sin(2.0 * np.pi * pf.data[SeasonalTags.day_of_year] / days_in_year)
            pf.data["time_of_year_cos"] = np.cos(2.0 * np.pi * pf.data[SeasonalTags.day_of_year] / days_in_year)
