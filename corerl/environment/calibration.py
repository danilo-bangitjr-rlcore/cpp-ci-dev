...
class CalibrationEnv(gym.Env):
    ...
    is fully observable.
    """
    def __init__(self, cfg: dict | CalibrationConfig | None = None):
        if isinstance(cfg, dict):
            _cfg = config_from_dict(CalibrationConfig, cfg)
            assert isinstance(_cfg, CalibrationConfig)
            self._cfg = _cfg
        elif cfg is None:
...
gym.register(
    id='Calibration-v0',
    entry_point='corerl.environment.calibration:CalibrationEnv',
)
