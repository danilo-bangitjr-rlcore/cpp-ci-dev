...
def df(cfg: MainConfig, tag_names: list[str]):
    ...
    data["SCADA1.ELS_SLR_ELSP72_POWER_PEAK_SP_LGC.F_CV"] = 4400
...
def test_ENERGY_SOLD(cfg: MainConfig, app_state: AppState, tag_names: list[str]):
    ...
    # '({Actual_Posted_Pool_Price} - {SCADA1.ELS_SLR_ENG_EXPORT_FEE_SP.F_CV}) * {SCADA1.ELS_SLR_ELSP72_ENG_OUT_TOT.F_CV}'
    # (111.11 - 0.375) * 5.9 = 653.3365
...
def test_PROFIT(cfg: MainConfig, app_state: AppState, tag_names: list[str]):
    ...
    # 653.3365 - 279.778 - 25.0 = 348.5585
    assert np.isnan(out.data["PROFIT"].iloc[0])
    assert np.isclose(out.data["PROFIT"].iloc[1], 348.5585)
...
def test_curtailment_violation(cfg: MainConfig, app_state: AppState, df: pd.DataFrame):
    ...
    # Red/Yellow Zone Tags
    df["SCADA1.ELS_SLR_ELSP72_KW.F_CV"] = 4750 # Curtailment yellow zone violation
...
def test_power_peak_yellow_zone_violation(cfg: MainConfig, app_state: AppState, df: pd.DataFrame):
    ...
    df["SCADA1.ELS_SLR_ELSP72_ENG_IN_TOT.F_CV"] = 0.0
    for step in range(obs_steps):
        df.loc[dates[step], "SCADA1.ELS_SLR_ELSP72_ENG_IN_TOT.F_CV"] += 0.35 * step # yellow zone violation
...
def test_power_peak_red_zone_violation(cfg: MainConfig, app_state: AppState, df: pd.DataFrame):
    ...
    df["SCADA1.ELS_SLR_ELSP72_ENG_IN_TOT.F_CV"] = 0.0
    for step in range(obs_steps):
        df.loc[dates[step], "SCADA1.ELS_SLR_ELSP72_ENG_IN_TOT.F_CV"] += 0.4 * step  # red zone violation
    ...
    # Expected
    # SCADA1.ELS_SLR_ELSP72_ENG_IN_TOT.F_CV = 0.4 represents 5.2631% red zone violation
...
def test_on_site_consumption_yellow_zone_violation(cfg: MainConfig, app_state: AppState, df: pd.DataFrame):
    ...
    # Red/Yellow Zone Tags
    df["SCADA1.ELS_SLR_ELSP72_MAJORITY_LGC.F_CV"] = 50 # On-site consumption yellow zone violation
...
def test_on_site_consumption_red_zone_violation(cfg: MainConfig, app_state: AppState, df: pd.DataFrame):
    ...
    # Red/Yellow Zone Tags
    df["SCADA1.ELS_SLR_ELSP72_MAJORITY_LGC.F_CV"] = 48 # On-site consumption red zone violation
