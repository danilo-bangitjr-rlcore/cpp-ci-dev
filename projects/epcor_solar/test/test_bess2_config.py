...
def cfg():
    main_cfg = direct_load_config(
        MainConfig,
        config_name='epcor_solar/configs/bess2.yaml',
    )
...
def df(cfg: MainConfig, tag_names: list[str]):
    ...
    data["ELS_SLR_ELSP12_POWER_PEAK_SP_LGC"] = 4.4
    data["ELS_SLR_ELSP12_KW"] = 4000
    data["ELS_SLR_ELSP12_MAJORITY_LGC"] = 100
    data["ELS_SLR_ELSP12_ENG_IN_TOT"] *= 0.0
    for step in range(obs_steps):
        data.loc[dates[step], "ELS_SLR_ELSP12_ENG_IN_TOT"] += 0.2 * step

    data["ELS_SLR_BESS2_ENG_OUT_TOT"] = 0
...
def test_P12_MAJORITY_ON_SITE_CONSUMPTION_CONSTRAINT_early_in_year(
    ...
    df["ELS_SLR_ELSP12_MAJORITY_LGC"] = 85.7
    ...
    # '(100 - {ELS_SLR_ELSP12_MAJORITY_LGC}) * (1 / (1 + 2.71828**(-1*(({day_of_year} - 250) / 20))))'
    # {ELS_SLR_ELSP12_MAJORITY_LGC} = 85.7
    # {day_of_year} = 45
    # (1 / (1 + 2.71828**(-1*((45 - 250) / 20)))) = 0.000035
    # (100 - 85.7) * 0.000035 = 0.0005005
    assert np.isclose(out.data["P12_MAJORITY_ON_SITE_CONSUMPTION_CONSTRAINT"], 0.0005005, atol=1e-4)
...
def test_P12_MAJORITY_ON_SITE_CONSUMPTION_CONSTRAINT_mid_year(
    ...
    df["ELS_SLR_ELSP12_MAJORITY_LGC"] = 20.0
    pf = PipelineFrame(df, DataMode.ONLINE)
    ...
    # '(100 - {ELS_SLR_ELSP12_MAJORITY_LGC}) * (1 / (1 + 2.71828**(-1*(({day_of_year} - 250) / 20))))'
    # {ELS_SLR_ELSP12_MAJORITY_LGC} = 20
    # {day_of_year} = 175
    # (1 / (1 + 2.71828**(-1*((175 - 250) / 20)))) = 0.022977
    # (100 - 20) * 0.022977 = 1.83816
    assert np.isclose(out.data["P12_MAJORITY_ON_SITE_CONSUMPTION_CONSTRAINT"], 1.83816, atol=1e-4)
...
def test_P12_MAJORITY_ON_SITE_CONSUMPTION_CONSTRAINT_end_of_year(
    ...
    df["ELS_SLR_ELSP12_MAJORITY_LGC"] = 50.0
    pf = PipelineFrame(df, DataMode.ONLINE)
    ...
    # '(100 - {ELS_SLR_ELSP12_MAJORITY_LGC}) * (1 / (1 + 2.71828**(-1*(({day_of_year} - 250) / 20))))'
    # {ELS_SLR_ELSP12_MAJORITY_LGC} = 50
    # {day_of_year} = 345
    # (1 / (1 + 2.71828**(-1*((345 - 250) / 20)))) = 0.991422
    # (100 - 50) * 0.991422 = 49.5711
    assert np.isclose(out.data["P12_MAJORITY_ON_SITE_CONSUMPTION_CONSTRAINT"], 49.5711, atol=1e-4)
...
def test_BESS_DEGRADATION_COST(cfg: MainConfig, app_state: AppState, tag_names: list[str]):
    ...
    df.loc[dates[0], "ELS_SLR_BESS2_ENG_IN_TOT"] = 100.0
    df.loc[dates[1], "ELS_SLR_BESS2_ENG_IN_TOT"] = 105.0
    df.loc[dates[0], "ELS_SLR_BESS2_ENG_OUT_TOT"] = 100.0
    df.loc[dates[1], "ELS_SLR_BESS2_ENG_OUT_TOT"] = 100.0
    pf = PipelineFrame(df, DataMode.ONLINE)

    pf = seasonal_tag_stage(pf)
    pf = delta_tag_stage(pf)
    assert np.isnan(pf.data["ELS_SLR_BESS2_ENG_IN_TOT"].iloc[0])
    assert np.isclose(pf.data["ELS_SLR_BESS2_ENG_IN_TOT"].iloc[1], 5.0)
    assert np.isnan(pf.data["ELS_SLR_BESS2_ENG_OUT_TOT"].iloc[0])
    assert np.isclose(pf.data["ELS_SLR_BESS2_ENG_OUT_TOT"].iloc[1], 0.0)

    out = virtual_tag_stage(pf)

    # '{ELS_SLR_ENG_BATT_COST_SP} * ({ELS_SLR_BESS2_ENG_IN_TOT} + {ELS_SLR_BESS2_ENG_OUT_TOT})'
    # 5 * (5 + 0) = 25
...
def test_CHARGING_FROM_GRID(cfg: MainConfig, app_state: AppState, tag_names: list[str]):
    ...
    for i in range(len(dates)):
        df.loc[dates[i], "ELS_SLR_SOLAR2_ENG_OUT_TOT"] = 100.0 + (i * 0.5)
        df.loc[dates[i], "ELS_SLR_WTP2_ENG_IN_TOT"] = 100.0 + (i**2 * 0.2)
        df.loc[dates[i], "ELS_SLR_BESS2_ENG_IN_TOT"] = 100.0 + (i * 0.1)
    pf = PipelineFrame(df, DataMode.ONLINE)
    ...
    # CHARGING_FROM_GRID: '({ELS_SLR_SOLAR2_ENG_OUT_TOT} <= {ELS_SLR_WTP2_ENG_IN_TOT}) & ({ELS_SLR_BESS2_ENG_IN_TOT} > 0)'
    # (0.5 <= 0.3) & (0.1 > 0): False
    # (0.5 <= 0.9) & (0.1 > 0): True
...
def test_CHARGING_FROM_SOLAR(cfg: MainConfig, app_state: AppState, tag_names: list[str]):
    ...
    # CHARGING_FROM_SOLAR: '(({ELS_SLR_SOLAR2_ENG_OUT_TOT} > {ELS_SLR_WTP2_ENG_IN_TOT}) & ({ELS_SLR_BESS2_ENG_IN_TOT} > 0)) & ({ELS_SLR_ELSP12_KW} < 4875)'
...
def test_CHARGING_FROM_GRID_COST(cfg: MainConfig, app_state: AppState, tag_names: list[str]):
    ...
    df["ELS_SLR_ENG_BATT_COST_SP"] = 5.0
    df["ELS_SLR_BESS2_ENG_OUT_TOT"] = 0.0
    for i in range(len(dates)):
        df.loc[dates[i], "ELS_SLR_SOLAR2_ENG_OUT_TOT"] = 100.0 + (i * 0.5)
        df.loc[dates[i], "ELS_SLR_BESS2_ENG_IN_TOT"] = 100.0 + (i * 0.1)
        df.loc[dates[i], "ELS_SLR_WTP2_ENG_IN_TOT"] = 100.0 + (i**2 * 0.2)
    ...
    # CHARGING_FROM_GRID_COST: '{CHARGING_FROM_GRID} * {ELS_SLR_BESS2_ENG_IN_TOT} * {ELS_SLR_ENG_IMPORT_COST_SP}'
    # False * 0.1 * 47.42 = 0.0
...
def test_CHARGING_FROM_SOLAR_COST(cfg: MainConfig, app_state: AppState, tag_names: list[str]):
    ...
    # CHARGING_FROM_SOLAR_COST: '{CHARGING_FROM_SOLAR} * {ELS_SLR_BESS2_ENG_IN_TOT} * {Actual_Posted_Pool_Price}'
    # True * 0.1 * 100.0 = 10.0
    # False * 0.1 * 100.0 = 0.0
    # BESS_DEGRADATION: 0.1 * 5.0 = 0.5
...
def test_ALL_ENERGY_DISCHARGED_POOL_PRICE(cfg: MainConfig, app_state: AppState, tag_names: list[str]):
    ...
    for i in range(len(dates)):
        df.loc[dates[i], "ELS_SLR_SOLAR2_ENG_OUT_TOT"] = 100.0 + (i * 0.5)
        df.loc[dates[i], "ELS_SLR_WTP2_ENG_IN_TOT"] = 100.0 + (i**2 * 0.2)
        df.loc[dates[i], "ELS_SLR_BESS2_ENG_OUT_TOT"] = 100.0 + (i * 0.1)
    pf = PipelineFrame(df, DataMode.ONLINE)
    ...
    # ALL_ENERGY_DISCHARGED_POOL_PRICE: '({ELS_SLR_SOLAR2_ENG_OUT_TOT} > {ELS_SLR_WTP2_ENG_IN_TOT}) & ({ELS_SLR_BESS2_ENG_OUT_TOT} > 0)'
    # (0.5 > 0.3) & (0.1 > 0): True
    # (0.5 > 0.9) & (0.1 > 0): False
...
def test_ALL_ENERGY_DISCHARGED_GRID_PRICE(cfg: MainConfig, app_state: AppState, tag_names: list[str]):
    ...
    for i in range(len(dates)):
        df.loc[dates[i], "ELS_SLR_SOLAR2_ENG_OUT_TOT"] = 100.0 + (i * 0.5)
        df.loc[dates[i], "ELS_SLR_BESS2_ENG_OUT_TOT"] = 100.0 + (i * 0.05)
        df.loc[dates[i], "ELS_SLR_WTP2_ENG_IN_TOT"] = 100.0 + (i**2 * 0.2)
    pf = PipelineFrame(df, DataMode.ONLINE)
    ...
    # ALL_ENERGY_DISCHARGED_GRID_PRICE: '(({ELS_SLR_SOLAR2_ENG_OUT_TOT} + {ELS_SLR_BESS2_ENG_OUT_TOT}) <= {ELS_SLR_WTP2_ENG_IN_TOT}) & ({ELS_SLR_BESS2_ENG_OUT_TOT} > 0)'
...
def test_SOME_ENERGY_DISCHARGED_POOL_PRICE(cfg: MainConfig, app_state: AppState, tag_names: list[str]):
    ...
    df = pd.DataFrame(data=np.ones((3, len(tag_names))), columns=tag_names, index=dates)
    for i in range(len(dates)):
        df.loc[dates[i], "ELS_SLR_SOLAR2_ENG_OUT_TOT"] = 100.0 + (i * 0.475)
        df.loc[dates[i], "ELS_SLR_BESS2_ENG_OUT_TOT"] = 100.0 + (i * 0.05)
        df.loc[dates[i], "ELS_SLR_WTP2_ENG_IN_TOT"] = 100.0 + (i**2 * 0.5)
    pf = PipelineFrame(df, DataMode.ONLINE)
    ...
    # SOME_ENERGY_DISCHARGED_POOL_PRICE: '((({ELS_SLR_SOLAR2_ENG_OUT_TOT} + {ELS_SLR_BESS2_ENG_OUT_TOT}) > {ELS_SLR_WTP2_ENG_IN_TOT}) & ({ELS_SLR_SOLAR2_ENG_OUT_TOT} < {ELS_SLR_WTP2_ENG_IN_TOT})) & ({ELS_SLR_BESS2_ENG_OUT_TOT} > 0)'
...
def test_DISCHARGED_POOL_PRICE_REVENUE_1(cfg: MainConfig, app_state: AppState, tag_names: list[str]):
    ...
    df["ELS_SLR_ENG_IMPORT_COST_SP"] = 47.42
    df["ELS_SLR_ENG_BATT_COST_SP"] = 5.0
    df["ELS_SLR_BESS2_ENG_IN_TOT"] = 0.0
    for i in range(len(dates)):
        df.loc[dates[i], "ELS_SLR_SOLAR2_ENG_OUT_TOT"] = 100.0 + (i * 0.5)
        df.loc[dates[i], "ELS_SLR_BESS2_ENG_OUT_TOT"] = 100.0 + (i * 0.1)
        df.loc[dates[i], "ELS_SLR_WTP2_ENG_IN_TOT"] = 100.0 + (i**2 * 0.2)
    ...
    # DISCHARGED_POOL_PRICE_REVENUE: '({ALL_ENERGY_DISCHARGED_POOL_PRICE} * {ELS_SLR_BESS2_ENG_OUT_TOT} * ({Actual_Posted_Pool_Price} - {ELS_SLR_ENG_EXPORT_FEE_SP})) + ({SOME_ENERGY_DISCHARGED_POOL_PRICE} * ({ELS_SLR_SOLAR2_ENG_OUT_TOT} + {ELS_SLR_BESS2_ENG_OUT_TOT} - {ELS_SLR_WTP2_ENG_IN_TOT}) * ({Actual_Posted_Pool_Price} - {ELS_SLR_ENG_EXPORT_FEE_SP}))'
...
def test_DISCHARGED_GRID_PRICE_REVENUE_1(cfg: MainConfig, app_state: AppState, tag_names: list[str]):
    ...
    # DISCHARGED_GRID_PRICE_REVENUE: '({ALL_ENERGY_DISCHARGED_GRID_PRICE} * {ELS_SLR_BESS2_ENG_OUT_TOT} * {ELS_SLR_ENG_IMPORT_COST_SP}) + ({SOME_ENERGY_DISCHARGED_POOL_PRICE} * ({ELS_SLR_WTP2_ENG_IN_TOT} - {ELS_SLR_SOLAR2_ENG_OUT_TOT}) * {ELS_SLR_ENG_IMPORT_COST_SP})'
...
def test_curtailment_violation(cfg: MainConfig, app_state: AppState, df: pd.DataFrame):
    ...
    # Red/Yellow Zone Tags
    df["ELS_SLR_ELSP12_KW"] = 4750 # Curtailment yellow zone violation
    ...
    # Normalized PROFIT = 0.764687
    # Scaled to [-1, 0]: -0.235313
    # ELS_SLR_ELSP12_KW = 4750 represents 16.67% yellow zone violation
...
def test_power_peak_yellow_zone_violation(cfg: MainConfig, app_state: AppState, df: pd.DataFrame):
    ...
    # Red/Yellow Zone Tags
    # Red Bound: '{ELS_SLR_ELSP12_POWER_PEAK_SP_LGC} * 5 / 60' = 4.4 * 5 / 60 = 0.36667
    # Yellow Bound: 0.95 * 0.36667 = 0.3483
    df["ELS_SLR_ELSP12_ENG_IN_TOT"] = 0.0
    for step in range(obs_steps):
        df.loc[dates[step], "ELS_SLR_ELSP12_ENG_IN_TOT"] += 0.35 * step # yellow zone violation
    ...
    # Normalized PROFIT = 0.764687
    # Scaled to [-1, 0]: -0.235313
    # ELS_SLR_ELSP12_ENG_IN_TOT = 0.35 represents 9.0909% yellow zone violation
...
def test_power_peak_red_zone_violation(cfg: MainConfig, app_state: AppState, df: pd.DataFrame):
    ...
    df["ELS_SLR_ELSP12_ENG_IN_TOT"] = 0.0
    for step in range(obs_steps):
        df.loc[dates[step], "ELS_SLR_ELSP12_ENG_IN_TOT"] += 0.4 * step  # red zone violation
    ...
    # Expected
    # ELS_SLR_ELSP12_ENG_IN_TOT = 0.4 represents 5.2631% red zone violation
...
def test_on_site_consumption_yellow_zone_violation(cfg: MainConfig, app_state: AppState, df: pd.DataFrame):
    ...
    # Red/Yellow Zone Tags
    df["ELS_SLR_ELSP12_MAJORITY_LGC"] = 50 # On-site consumption yellow zone violation
...
def test_on_site_consumption_red_zone_violation(cfg: MainConfig, app_state: AppState, df: pd.DataFrame):
    ...
    # Red/Yellow Zone Tags
    df["ELS_SLR_ELSP12_MAJORITY_LGC"] = 48 # On-site consumption red zone violation
