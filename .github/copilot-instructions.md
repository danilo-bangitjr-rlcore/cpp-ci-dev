...
4. **Documentation**: Use docstrings only for public methods/classes. Internal methods should not require documentation.
5. **Types**: Use type hints for inputs, prefer generic/wide types, avoid `Any`, and rely on inference where possible. Assume modern python versions (3.13+).
6. **Minimal changes**: Prefer making minimal changes to existing code. If larger changes are needed, consider staging them in smaller, incremental commits.
7. **Testability**: Write code that is easy to test. Avoid complex dependencies and side effects. Use dependency injection where appropriate.
