...
from corerl.sql_logging.base_schema import SQLTransition, Base, NetworkWeights
...
class TestORMSchema(unittest.TestCase):
    ...
    def test_critic_weight_logging(self):
        ...
        df = pd.read_sql(
            select(NetworkWeights).order_by(NetworkWeights.ts.desc()).limit(1),
            con=self.engine,
        )
