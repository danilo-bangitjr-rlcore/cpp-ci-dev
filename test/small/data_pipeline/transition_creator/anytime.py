...
def test_anytime_3():
    ...
    assert torch.equal(t_1.next_state, Tensor([4.]))
    assert not t_1.terminated
    assert not t_1.truncate
    ...
    assert torch.equal(t_2.next_state, Tensor([4.]))
    ...
    assert torch.equal(t_3.next_state, Tensor([4.]))
