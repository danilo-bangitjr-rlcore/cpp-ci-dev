...
def test_anytime_3():
    ...
    assert t_2.n_steps == 3
    assert t_2.n_step_reward == 2.71
    assert torch.equal(t_2.next_state, Tensor([5.]))
    assert not t_2.terminated
    assert not t_2.truncate

    assert torch.equal(t_3.state, Tensor([3.]))
    assert torch.equal(t_3.action, Tensor([1.]))
    assert t_3.n_steps == 2
    assert t_3.n_step_reward == 1.9
    assert torch.equal(t_3.next_state, Tensor([5.]))
    assert not t_3.terminated
    assert not t_3.truncate

    assert torch.equal(t_4.state, Tensor([4.]))
    assert torch.equal(t_4.action, Tensor([1.]))
    assert t_4.n_steps == 1
    assert t_4.n_step_reward == 1.
    assert torch.equal(t_4.next_state, Tensor([5.]))
    assert not t_4.terminated
    assert not t_4.truncate
