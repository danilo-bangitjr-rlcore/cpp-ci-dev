...
class TestWriteNodes:
    ...
    def test_write_nodes_no_engine(self, sql_manager: Mock):
        sql_manager.engine = None

        with patch('coreio.communication.sql_communication.logger') as mock_logger:
            sql_manager.write_nodes({"sensor1": 1.0}, "2023-01-01T00:00:00Z")
            mock_logger.error.assert_called_once_with("SQL engine is not initialized")

    def test_write_nodes_no_data(self, sql_manager: SQL_Manager):
        with patch('coreio.communication.sql_communication.logger') as mock_logger:
            sql_manager.write_nodes({}, "2023-01-01T00:00:00Z")
            mock_logger.warning.assert_called_once_with("No data provided to write_nodes")

    def test_write_nodes_no_valid_columns(self, sql_manager: SQL_Manager):
        with patch('coreio.communication.sql_communication.logger') as mock_logger:
            sql_manager.write_nodes({"invalid_node": 1.0}, "2023-01-01T00:00:00Z")
            mock_logger.warning.assert_called_once_with("No valid columns found in provided data")

    def test_write_nodes_filters_and_inserts(self, sql_manager: SQL_Manager):
        connection = Mock()
        with patch('coreio.communication.sql_communication.TryConnectContextManager') as mock_context:
            ...
            sql_manager.write_nodes({"sensor1": 1.0, "invalid": 2.0, "sensor2": 42}, "2023-01-01T00:00:00Z")
    ...
    def test_write_nodes_uses_default_timestamp(self, mock_now_iso: Mock, sql_manager: SQL_Manager):
        ...
        with patch('coreio.communication.sql_communication.TryConnectContextManager') as mock_context:
            ...
            sql_manager.write_nodes({"sensor1": 1.0}, None)

            args, _ = mock_connection.execute.call_args
            executed_data = args[1]
            assert executed_data["timestamp"] == "2023-01-01T12:00:00Z"

    def test_write_nodes_uses_provided_timestamp(self, sql_manager: SQL_Manager):
        mock_connection = Mock()
        ...
        with patch('coreio.communication.sql_communication.TryConnectContextManager') as mock_context:
            ...
            sql_manager.write_nodes({"sensor1": 1.0}, timestamp)

            args, _ = mock_connection.execute.call_args
            executed_data = args[1]
            assert executed_data["timestamp"] == timestamp

    def test_write_nodes_case_insensitive_keys(self, sql_manager: SQL_Manager):
        mock_connection = Mock()
        ...
        with patch('coreio.communication.sql_communication.TryConnectContextManager') as mock_context:
            ...
            sql_manager.write_nodes(data, "2023-01-01T00:00:00Z")
    ...
    def test_write_nodes_commits_transaction(self, sql_manager: SQL_Manager):
        mock_connection = Mock()
    ...
    def test_write_nodes_handles_database_exception(self, sql_manager: SQL_Manager):
        mock_connection = Mock()
