...
SWEEP_PARAMS = {
    'independent': {
        ...
        'interaction/action_normalizer': ['scale'],
        'interaction/reward_normalizer': ['scale'],
        'experiment.exp_name': ["Fixed_Data_Loader_Transitions_Composite_Alert_Sweep"],
        'experiment.max_steps': [50000],
        'experiment.offline_steps': [20000],
        'experiment.set_env_obs_space': [True],
    ...
    'conditional': {
        ...
        'interaction.action_normalizer.action_high': lambda d: [300.0] if d['interaction/action_normalizer'] == 'scale' else None,
        'interaction.action_normalizer.action_low': lambda d: [0.0] if d['interaction/action_normalizer'] == 'scale' else None,
        'interaction.reward_normalizer.reward_high': lambda d: [4.0] if d['interaction/reward_normalizer'] == 'scale' else None,
        'interaction.reward_normalizer.reward_low': lambda d: [-9.0] if d['interaction/reward_normalizer'] == 'scale' else None,
        'interaction.reward_normalizer.reward_bias': lambda d: [4.0] if d['interaction/reward_normalizer'] == 'scale' else None,
        ...
        'data_loader.offline_data_path': lambda d: ["offline_data"] if d['env'] == 'reseau' else None,
        'data_loader.skip_rows': lambda d: [3] if d['env'] == 'reseau' else None,
        'data_loader.header': lambda d: [0] if d['env'] == 'reseau' else None,
    }
}
