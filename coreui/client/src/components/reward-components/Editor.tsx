...
class PriorityNode extends ClassicPreset.Node<
  { threshold: ClassicPreset.Socket; expression: ClassicPreset.Socket },
  { value: ClassicPreset.Socket },
  { value: ClassicPreset.InputControl<"number"> }
> {
  height = 190;
  ...
  constructor(
    ...
    this.addInput("threshold", left);
    this.addInput("expression", right);
    this.addControl(
      "value",
  ...
  data(inputs: { threshold?: number[]; expression?: number[] }): { value: number } {
    const leftControl = this.inputs.threshold?.control as ClassicPreset.InputControl<
      "number"
    >;
...
type Node = NumberNode | AddNode | PriorityNode;
...
export async function createEditor(container: HTMLElement) {
  ...
  dock.add(() => new AddNode(process, (c) => area.update("control", c.id)))
  ...
  const x = new PriorityNode(process, (c) => area.update("control", c.id));
