...
def critic(critic_config: AdvConfig) -> AdvCritic:
    state_dim = 4
    action_dim = 2
    return AdvCritic(critic_config, state_dim, action_dim)
...
def test_forward_values_single_sample(critic: AdvCritic):
    """
...
def test_forward_values_batch(critic: AdvCritic):
    """
    Given a valid critic state, an observation, and an action, the critic
    should produce a valid advantage value.
...
def test_forward_values_batch_n_samples(critic: AdvCritic):
    """
    Given a valid critic state, an observation, and an action, the critic
    should produce a valid advantage value.
...
def rolling_critic(rolling_reset_config: AdvConfig) -> AdvCritic:
    state_dim = 4
    action_dim = 2
    return AdvCritic(rolling_reset_config, state_dim, action_dim)
...
def test_get_active_values(rolling_critic: AdvCritic):
    """Test that forward only returns values for active critics."""
    state = jnp.array([1.0, 2.0, 3.0, 4.0])
    action = jnp.array([0.5, -0.5])
    rng = jax.random.PRNGKey(123)
    ...
    rng_apply = jax.random.PRNGKey(456)
    active_values = rolling_critic.forward(critic_state.params, rng_apply, state, action).adv
...
def test_update(critic: AdvCritic):
    ...
    new_state, metrics = critic.update(update_rng, critic_state, transitions, policy_actions, policy_probs)
