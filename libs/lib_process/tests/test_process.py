...
def test_start_in_background_with_stderr_forwarding():
    """
    Process.start_in_background forwards stderr when logger provided
    """
    ...
    assert process.is_running() or process.is_zombie()
    ...
    assert "Service stderr output" in str(call_args)
    assert "test error" in str(call_args)


def test_start_in_background_without_logger():
    """
    Process.start_in_background works without logger (backward compatibility)
    """
    process = Process.start_in_background(args=[sys.executable, "-c", "import time; time.sleep(0.1)"])
...
def test_start_in_background_continuous_stderr():
    """
    Process.start_in_background handles continuous stderr without hanging
    """
    from unittest.mock import MagicMock
    ...
    assert mock_logger.error.call_count >= 15
